...
export function SMSLogTable({ logs, isLoading, onResend }: SMSLogTableProps) {
  ...
  const handleExportCSV = () => {
    ...
    const blob = new (window.Blob || (window as any).Blob)([csvContent], { type: 'text/csv' });
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  ...
                  {log.status === 'failed' && log.provider_response && (
                    <button
                      ...
                      title={typeof log.provider_response === 'string' ? log.provider_response : (typeof log.provider_response === 'object' ? JSON.stringify(log.provider_response) : 'Error details available')}
                    >
                      View Error
                    </button>
                  )}
