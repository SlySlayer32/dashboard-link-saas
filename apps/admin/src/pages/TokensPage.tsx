...
export function TokensPage() {
  ...
  const handleRevoke = async (tokenId: string) => {
    if (!globalThis.window.confirm('Are you sure you want to revoke this token?')) {
      return
    }
      ...
      { workerId, expiryHours: 24 },
      {
        onSuccess: (response) => {
          ...
          if (response.data?.token?.token) {
            ...
            window.alert('New token generated and copied to clipboard!')
          }
        },
        onError: (error: Error) => {
    ...
    <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
      ...
      <div className='sm:flex sm:items-center sm:justify-between mb-8'>
        <div>
          ...
          <p className='mt-1 text-sm text-gray-500'>Manage and monitor dashboard access tokens</p>
        </div>
