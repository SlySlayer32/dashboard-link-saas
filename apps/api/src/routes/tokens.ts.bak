...
tokens.get('/', zValidator('query', listTokensSchema), async (c) => {
  ...
  if (!organizationId) {
    return c.json({ success: false, error: 'Organization not found in context' }, 401)
  }
  ...
  try {
    ...
    logger.error('Failed to list tokens', error instanceof Error ? error : new Error(String(error)))
    return c.json(
      {
        success: false,
...
tokens.post('/bulk-revoke-expired', async (c) => {
  ...
  try {
    ...
    logger.error(
      'Failed to bulk revoke expired tokens',
      error instanceof Error ? error : new Error(String(error))
    )
    return c.json(
      {
...
tokens.post(
  ...
  async (c) => {
    ...
    try {
      ...
      logger.error(
        'Failed to revoke worker sessions',
        error instanceof Error ? error : new Error(String(error))
      )
      return c.json(
        {
