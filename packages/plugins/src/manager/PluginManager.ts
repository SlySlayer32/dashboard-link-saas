...
export class PluginManagerImpl implements PluginManager {
  ...
  async executeSchedulePlugins(
    ...
    for (const config of configs) {
      try {
        const result = await pluginService.getSchedule(workerId, dateRange, config)
        results.push(result)
      } catch (error) {
        results.push({
          success: false,
  ...
  async executeTaskPlugins(
    ...
    for (const config of configs) {
      try {
        const result = await pluginService.getTasks(workerId, config)
        results.push(result)
      } catch (error) {
        results.push({
          success: false,
  ...
  async executeAllPlugins(
    ...
    const scheduleResults = await this.executeSchedulePlugins(workerId, configs, dateRange)
  ...
  private createBatchResult<T>(results: PluginResponse<T>[]): PluginBatchResult<T> {
    const executionResults: PluginExecutionResult<T>[] = results.map((result) => ({
    ...
    const aggregatedData = results.filter((r) => r.success).flatMap((r) => r.data)
