...
export class DatabaseTokenProvider extends BaseTokenProvider {
  ...
  async validate(token: string): Promise<TokenValidation> {
    try {
      ...
      if (!tokenRecord) {
        return this.createValidationResult(false, undefined, 'Token not found', 'NOT_FOUND')
      }

      // Check if token is revoked
      if (tokenRecord.revoked) {
        return this.createValidationResult(false, undefined, 'Token has been revoked', 'REVOKED')
      }

      // Check if token is expired
      if (new Date() > tokenRecord.expires_at) {
        return this.createValidationResult(false, undefined, 'Token has expired', 'EXPIRED')
      }
      ...
      try {
        ...
        return this.createValidationResult(false, undefined, 'Invalid token payload', 'INVALID')
      }
      ...
      return this.createValidationResult(false, undefined, this.formatError(error), 'INVALID')
    }
  }
  ...
  async getMetadata(token: string): Promise<Record<string, unknown> | null> {
    try {
      ...
      return tokenRecord.metadata
        ? (JSON.parse(tokenRecord.metadata) as Record<string, unknown>)
        : null
    } catch {
      return null
    }
  ...
  private async insertTokenRecord(
    record: Omit<DatabaseTokenRecord, 'id'>
  ): Promise<DatabaseTokenRecord> {
