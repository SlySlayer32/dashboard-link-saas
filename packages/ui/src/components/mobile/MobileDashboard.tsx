  ...
  return (
    ...
    >
      <CardContent className='p-4'>
        ...
        {dueDate && <div className='text-xs text-gray-500'>Due: {dueDate}</div>}
      </CardContent>
    </Card>
  )
}
...
export function MobileDashboard({
  ...
  return (
    <div className='min-h-screen bg-gray-50 p-4 pb-20'>
      ...
      <MobileSectionHeader title='Schedule' count={scheduleItems.length} />

      {scheduleItems.length > 0 ? (
        scheduleItems.map((item, index) => (
          <MobileScheduleItem key={index} {...item} onPress={() => onScheduleItemPress?.(item)} />
        ))
      ) : (
      ...
      {taskItems.length > 0 ? (
        taskItems
          .slice(0, 3)
          .map((item, index) => (
            <MobileTaskItem key={index} {...item} onPress={() => onTaskItemPress?.(item)} />
          ))
      ) : (
        <Card variant='muted' className='mb-6'>
          <CardContent className='p-4 text-center'>
