...
export const authProviderFactories: Record<string, AuthProviderFactory> = {
  ...
  supabase: (config: AuthConfig) => {
    ...
    return new SupabaseAuthProvider(client, config)
  },

  // Mock provider factory
  mock: (config: AuthConfig) => new MockAuthProvider(config),
...
export function registerDefaultAuthProvider(config: AuthConfig): AuthProvider | undefined {
  const providerName = config.provider || 'supabase'
  const provider = authProviderRegistry.createProvider(providerName, config)
  if (provider) {
    authProviderRegistry.register(providerName, provider)
  }
  return provider
}
