...
export class MobileMessageProvider extends BaseSMSProvider {
  ...
  async send(message: SMSMessage): Promise<SMSResult> {
    try {
      ...
      return this.createSuccessResult(result.message_id, result, result.cost, deliveryReport)
    } catch (error) {
  ...
  async validateConfig(config: SMSProviderConfig): Promise<SMSConfigValidationResult> {
    ...
    const password = typeof settings.password === 'string' ? settings.password : undefined
    const defaultSenderId =
      typeof settings.defaultSenderId === 'string' ? settings.defaultSenderId : undefined
    ...
    if (defaultSenderId && defaultSenderId.length > 11) {
