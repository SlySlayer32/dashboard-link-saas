...
export class TwilioProvider extends BaseSMSProvider {
  ...
  async validateConfig(config: SMSProviderConfig): Promise<SMSConfigValidationResult> {
    ...
    const accountSid = typeof settings.accountSid === 'string' ? settings.accountSid : undefined
    const authToken = typeof settings.authToken === 'string' ? settings.authToken : undefined
    const defaultFrom = typeof settings.defaultFrom === 'string' ? settings.defaultFrom : undefined
    ...
    } else if (!defaultFrom.startsWith('+')) {
  ...
  private async makeAPIRequest(
    ...
    const body = method === 'POST' && payload ? this.buildSearchParams(payload) : undefined
  ...
  private buildSearchParams(payload: Record<string, string | undefined>): string {
