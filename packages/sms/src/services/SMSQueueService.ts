...
export class SMSQueueService {
  // Phase 2: Replace in-memory queue with BullMQ + Redis, separate workers, retries/DLQ metrics.
  ...
  async dequeue(providerId?: string): Promise<SMSMessage | null> {
    ...
    for (const priority of this.priorityOrder) {
      ...
      for (let i = 0; i < availableMessages.length; i++) {
        ...
        if (
          queuedMsg.message.scheduledFor &&
          new Date(queuedMsg.message.scheduledFor) > new Date()
        ) {
          continue
        }
  ...
  async processQueue(
    ...
    try {
      ...
      while (processed < batchSize) {
        ...
        const results = await Promise.allSettled(messages.map((msg) => sendFunction(msg)))
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          if (result.status === 'fulfilled' && result.value.success) {
            ...
            const error =
              result.status === 'rejected' ? result.reason : (result.value as SMSResult).error
