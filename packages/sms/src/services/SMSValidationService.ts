  ...
  validateMessage(message: SMSMessage): ValidationResult {
    ...
    if (!message.body) {
      ...
      if (specialChars.test(message.body)) {
        warnings.push(
          'Message contains special characters which may reduce max length to 70 characters per segment'
        )
      }
    }
    ...
    if (message.from) {
      ...
      if (!fromValidation.valid) {
        ...
        if (!/^[a-zA-Z0-9]{1,11}$/.test(message.from)) {
          warnings.push(
            'Sender ID should be a valid phone number or alphanumeric (max 11 characters)'
          )
        }
      }
    }
    ...
    if (message.scheduledFor) {
      ...
      if (isNaN(scheduledDate.getTime())) {
        ...
        warnings.push(
          'Messages scheduled more than 7 days in advance may not be supported by all providers'
        )
      }
    }
