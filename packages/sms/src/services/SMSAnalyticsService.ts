import { SMSMessageStats, SMSCostStats, SMSDeliveryStats, SMSResult } from '@dashboard-link/shared'
...
export class SMSAnalyticsService {
  ...
  async getMessageStats(dateRange: DateRange): Promise<SMSMessageStats> {
    ...
    const totalDelivered = messages.filter(
      (m) => m.success && m.deliveryReport?.status === 'delivered'
    ).length
    const totalFailed = messages.filter((m) => !m.success).length

    // Calculate average delivery time
    const deliveredMessages = messages.filter((m) => m.success && m.deliveryReport?.deliveredAt)
    ...
    const averageDeliveryTime =
      deliveredMessages.length > 0 ? totalDeliveryTime / deliveredMessages.length : 0
  ...
  async getCostByProvider(dateRange: DateRange): Promise<SMSCostStats> {
    ...
    const averageCostPerMessage = messages.length > 0 ? totalCost / messages.length : 0
  ...
  async getDeliveryRates(dateRange: DateRange): Promise<SMSDeliveryStats> {
    ...
    const overallDeliveryRate = totalMessages > 0 ? (deliveredMessages / totalMessages) * 100 : 0
    ...
    for (const [provider, stats] of Object.entries(providerStats)) {
      deliveryRateByProvider[provider] = stats.sent > 0 ? (stats.delivered / stats.sent) * 100 : 0
    }
  ...
  async getCostOptimization(dateRange: DateRange): Promise<CostOptimizationResult> {
    ...
    const optimalProvider =
      Object.entries(costPerDelivery).sort(([, a], [, b]) => a - b)[0]?.[0] || 'unknown'
