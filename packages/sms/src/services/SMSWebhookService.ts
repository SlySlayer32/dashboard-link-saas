...
export class SMSWebhookService {
   ...
   */
  async processCallback(providerId: string, data: unknown): Promise<void> {
  ...
  getDeliveryReportsByStatus(status: DeliveryReport['status']): DeliveryReport[] {
    return Array.from(this.deliveryReports.values()).filter((report) => report.status === status)
  }
  ...
  getDeliveryReportsByProvider(providerId: string): DeliveryReport[] {
    return Array.from(this.deliveryReports.values()).filter(
...
export class TwilioWebhookHandler implements WebhookHandler {
  parseDeliveryReport(body: unknown): DeliveryReport {
    ...
    const status = this.mapTwilioStatus(
      (data.MessageStatus as string) || (data.SmsStatus as string)
    )
