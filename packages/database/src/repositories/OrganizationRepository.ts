...
import type { Organization, RepositoryFilter } from '@dashboard-link/shared'
...
export class OrganizationRepository extends BaseRepository<Organization> {
  ...
  async getOrganizationWithAdmins(organizationId: string): Promise<{
    ...
    try {
      ...
      const admins = await this.adapter.query('admins').where({ organizationId }).build()
  ...
  protected transformFromDB(row: unknown): Organization {
    ...
    return {
      ...
      settings: data.settings
        ? {
            smsSenderId: (data.settings as Record<string, unknown>).sms_sender_id as
              | string
              | undefined,
            defaultTokenExpiry: (data.settings as Record<string, unknown>).default_token_expiry as
              | number
              | undefined,
            customMetadata: (data.settings as Record<string, unknown>).custom_metadata as
